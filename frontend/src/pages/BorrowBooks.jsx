...
export default function BorrowBooks() {
  ...
  // Fetch books when search term or category changes
  useEffect(() => {
  ...
  useEffect(() => {
    if (visitor?.dob) {
      getRecommendedBooks(visitor.dob)
        .then((data) => {
          // Sort recommended alphabetically as well
          const sorted = [...data].sort((a, b) => a.title.localeCompare(b.title));
  ...
  const getFilteredBooks = () => {
    ...
    // Already sorted, but re-sort to be safe
    return [...list].sort((a, b) => a.title.localeCompare(b.title));
  };

  const handleBorrow = () => {
    if (selectedBook) {
      navigate("/mobile/confirm-borrow", { state: { book: selectedBook } });
    }
  };
  ...
  return (
    <>
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        {/* Tabs: Available / Recommended / Unavailable */}
